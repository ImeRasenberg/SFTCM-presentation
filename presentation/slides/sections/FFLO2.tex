\begin{frame}{Zeeman Splitting}
	\only<1->{
		...
		-V\sum_{k,k'}\psi^*_{k,\uparrow}\psi^*_{-k,\downarrow}\psi_{-k',\downarrow}\psi_{k',\uparrow}
		\]
		\vspace{0.2cm}
	}
	\begin{columns}
		...
		\only<3->{
			\begin{tikzpicture}
				...
				\only<3>{
					...
					node[midway, above] {$\mathbf{Q}$};c
				}
				\only<4->{
					\node[draw, fill=red, fill opacity=0.2, text opacity=1,
...
\begin{frame}{Auxiliary field}
	\only<1->{
	\[
	\Delta_{BCS} = V\sum_k \langle
	...
	\begin{split}
		\Delta_{FF} &= 
		V\sum_k \langle \psi_{\downarrow,Q-k}(\tau)\psi_{\uparrow,k}(\tau) \rangle\\
		&= V\sum_k \langle \psi_{\downarrow,-k}(\tau)\psi_{\uparrow,k}(\tau) \rangle\  e^{iQr}\\
		&= \Delta_{BCS}\  e^{iQr}
	\end{split}
	\]
	}
	\only<3->{
		\[
		\Delta_{FF} = \Delta_{BCS}\  e^{iQr}
		\]
	}
	\only<3->{
		\[
		\Delta_{LO} = \sum_{j} \Delta_{BCS}\  e^{iQ_jr}
		\]
	}
\end{frame}

\begin{frame}{Energy Competition}
	\begin{columns}
		...
		\begin{tikzpicture}
			...
			% small circle (spin down)
			\draw[very thick, blue] (-\d,0) circle (\Rsmall);
			\node[blue] at (-\d,-\Rsmall-0.2) {$\downarrow$};
